                ...
                if ((h++, 1 === t.nodeType)) {
                    if (t.hasAttributes()) {
                        ...
                        for (; n-- > 0; ) {
                            ...
                            t.removeAttribute(n);
                            const r = o.split(s);
                            this.parts.push({ type: "attribute", index: h, name: i, strings: r }), (c += r.length - 1);
                        }
                    }
                    ...
                    if (e.indexOf(i) >= 0) {
                        ...
                        for (let e = 0; e < a; e++) {
                            ...
                            i.insertBefore(n, t), this.parts.push({ type: "node", index: ++h });
                        }
                    ...
                    if (t.data === i) {
                        ...
                        (null !== t.previousSibling && h !== p) || (h++, e.insertBefore(l(), t)), (p = h), this.parts.push({ type: "node", index: h }), null === t.nextSibling ? (t.data = "") : (n.push(t), h--), c++;
                    } else {
                        let e = -1;
                        for (; -1 !== (e = t.data.indexOf(i, e + 1)); ) this.parts.push({ type: "node", index: -1 }), c++;
                    }
...
class A {
    ...
    commit() {
        ...
        i && this.element.removeEventListener(this.eventName, this.__boundHandleEvent, this.__options),
            n && ((this.__options = T(t)), this.element.addEventListener(this.eventName, this.__boundHandleEvent, this.__options)),
            (this.value = t),
 ...
 */ function $(t) {
    let e = M.get(t.type);
    void 0 === e && ((e = { stringsArray: new WeakMap(), keyString: new Map() }), M.set(t.type, e));
...
void 0 === window.ShadyCSS
    ? (z = !1)
    : void 0 === window.ShadyCSS.prepareTemplateDom && (console.warn("Incompatible ShadyCSS version detected. Please update to at least @webcomponents/webcomponentsjs@2.0.2 and @webcomponents/shadycss@1.3.1."), (z = !1));
const j = (t) => (e) => {
        const n = U(e.type, t);
        let s = M.get(n);
        void 0 === s && ((s = { stringsArray: new WeakMap(), keyString: new Map() }), M.set(n, s));
        ...
        if (void 0 !== t) {
            ...
            n._valueHasChanged(this[t], e, s.hasChanged)
                ? (this._changedProperties.has(t) || this._changedProperties.set(t, e),
                  !0 !== s.reflect || 16 & this._updateState || (void 0 === this._reflectingProperties && (this._reflectingProperties = new Map()), this._reflectingProperties.set(t, s)))
                : (i = !1);
        }
...
class X extends W {
    ...
    update(t) {
        const e = this.render();
        super.update(t),
            e !== Q && this.constructor.render(e, this.renderRoot, { scopeName: this.localName, eventContext: this }),
...
(X.finalized = !0),
    (X.render = (t, i, n) => {
        ...
        if (
            (((t, i, n) => {
                let s = B.get(i);
                void 0 === s && (e(i, i.firstChild), B.set(i, (s = new C(Object.assign({ templateFactory: $ }, n)))), s.appendInto(i)), s.setValue(t), s.commit();
            })(t, l, Object.assign({ templateFactory: j(s) }, n)),
            a)
        ) {
            const t = B.get(l);
            B.delete(l);
...
let et = class extends X {
    ...
    checkManifest(t) {
        t.icons && t.icons[0]
            ? t.name
                ? t.description || console.error("Your web manifest must have a description listed")
    ...
    async install() {
        if (this.deferredprompt) {
            ...
            if ("accepted" === (await this.deferredprompt.userChoice).outcome) {
                ...
                // document.getElementById('installComponent').classList.remove("buttonhidden");
                let t = new CustomEvent("hide");
    ...
    render() {
        return O`${
            ("standalone" in navigator && !1 === navigator.standalone) || (!0 !== this.usecustom && this.shouldShowInstall() && !1 === this.installed)
                ? O`<button part="openButton" id="openButton" @click="${() => this.openPrompt()}"><slot>${this.installbuttontext}</slot></button>`
                : null
        } ${
            !0 === this.openmodal
                ? O`<div id="installModalWrapper">${
                      this.openmodal ? O`<div id="background" @click="${() => this.cancel()}"></div>` : null
                  }<div id="installModal" part="installModal"><div id="headerContainer"><div id="logoContainer"><img src="${this.iconpath ? this.iconpath : this.manifestdata.icons[0].src}" alt="App Logo"><div id="installTitle"><h1>${
                      this.manifestdata.short_name || this.manifestdata.name
                  }</h1><p id="desc">${this.explainer}</p></div></div><button id="closeButton" @click="${() =>
                      this.cancel()}" aria-label="Close"><svg width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.33" fill-rule="evenodd" clip-rule="evenodd" d="M1.11932 0.357981C1.59693 -0.119327 2.37129 -0.119327 2.8489 0.357981L11.7681 9.27152L20.6873 0.357981C21.165 -0.119327 21.9393 -0.119327 22.4169 0.357981C22.8945 0.835288 22.8945 1.60916 22.4169 2.08646L13.4977 11L22.4169 19.9135C22.8945 20.3908 22.8945 21.1647 22.4169 21.642C21.9393 22.1193 21.165 22.1193 20.6873 21.642L11.7681 12.7285L2.8489 21.642C2.37129 22.1193 1.59693 22.1193 1.11932 21.642C0.641705 21.1647 0.641705 20.3908 1.11932 19.9135L10.0385 11L1.11932 2.08646C0.641705 1.60916 0.641705 0.835288 1.11932 0.357981Z" fill="#60656D"/></svg></button></div><div id="contentContainer"><div id="featuresScreenDiv">${
                      this.manifestdata.features ? O`<div id="keyFeatures"><h3>${this.featuresheader}</h3><ul>${this.manifestdata.features ? this.manifestdata.features.map((t) => O`<li>${t}</li>`) : null}</ul></div>` : null
                  } ${
                      this.manifestdata.screenshots
                          ? O`<div id="screenshotsContainer"><button @click="${() =>
                                this.scrollToLeft()}" aria-label="previous image"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M401.4 224h-214l83-79.4c11.9-12.5 11.9-32.7 0-45.2s-31.2-12.5-43.2 0L89 233.4c-6 5.8-9 13.7-9 22.4v.4c0 8.7 3 16.6 9 22.4l138.1 134c12 12.5 31.3 12.5 43.2 0 11.9-12.5 11.9-32.7 0-45.2l-83-79.4h214c16.9 0 30.6-14.3 30.6-32 .1-18-13.6-32-30.5-32z"/></svg></button><section id="screenshots">${this.manifestdata.screenshots.map(
                                (t) => O`<div><img alt="App Screenshot" src="${t.src}"></div>`
                            )}</section><button @click="${() =>
                                this.scrollToRight()}" aria-label="next image"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M284.9 412.6l138.1-134c6-5.8 9-13.7 9-22.4v-.4c0-8.7-3-16.6-9-22.4l-138.1-134c-12-12.5-31.3-12.5-43.2 0-11.9 12.5-11.9 32.7 0 45.2l83 79.4h-214c-17 0-30.7 14.3-30.7 32 0 18 13.7 32 30.6 32h214l-83 79.4c-11.9 12.5-11.9 32.7 0 45.2 12 12.5 31.3 12.5 43.3 0z"/></svg></button></div>`
                          : null
                  }</div><div id="descriptionWrapper"><h3>${this.descriptionheader}</h3><p id="manifest-description">${this.manifestdata.description}</p></div></div>${
                      this.isIOS
                          ? O`<p id="iosText">${this.iosinstallinfotext}</p>`
                          : O`<div id="buttonsContainer">${
                                this.deferredprompt
                                    ? O`<button id="installButton" @click="${() => this.install()}">${this.installbuttontext} ${this.manifestdata.short_name}</button>`
                                    : O`<button @click="${() => this.cancel()}" id="installCancelButton">${this.cancelbuttontext}</button>`
                            }</div>`
                  }</div></div>`
                : null
        }`;
...
var it;
tt([J({ type: String })], et.prototype, "manifestpath", void 0),
    tt([J({ type: String })], et.prototype, "iconpath", void 0),
    tt([J({ type: Object })], et.prototype, "manifestdata", void 0),
    tt([J({ type: Boolean })], et.prototype, "openmodal", void 0),
    tt([J({ type: Boolean })], et.prototype, "showopen", void 0),
    tt([J({ type: Boolean })], et.prototype, "isSupportingBrowser", void 0),
    tt([J({ type: Boolean })], et.prototype, "isIOS", void 0),
    tt([J({ type: Boolean })], et.prototype, "installed", void 0),
    tt([J({ type: Boolean })], et.prototype, "hasprompt", void 0),
    tt([J({ type: Boolean })], et.prototype, "usecustom", void 0),
    tt([J({ type: Array })], et.prototype, "relatedApps", void 0),
    tt([J({ type: String })], et.prototype, "explainer", void 0),
    tt([J({ type: String })], et.prototype, "featuresheader", void 0),
    tt([J({ type: String })], et.prototype, "descriptionheader", void 0),
    tt([J({ type: String })], et.prototype, "installbuttontext", void 0),
    tt([J({ type: String })], et.prototype, "cancelbuttontext", void 0),
    tt([J({ type: String })], et.prototype, "iosinstallinfotext", void 0),
    tt([J()], et.prototype, "deferredprompt", void 0),
    (et = tt(
        [
            ((it = "pwa-install"),
            (t) =>
